...
utilities g_Utils;
using unique_handle = std::unique_ptr<void, unique_handle_deleter>;

ULONG utilities::get_pid(std::string _process)
{
	unique_handle Snapshot(CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, NULL));
	...
	if (!Process32First(Snapshot.get(), &_pEntry))
		return NULL;
	...
	} while (Process32Next(Snapshot.get(), &_pEntry));
...
{
	uintptr_t modBase = 0;
	unique_handle Snapshot(CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, _processId));
	...
	if (Snapshot.get() != INVALID_HANDLE_VALUE)
	{
		...
		if (Module32First(Snapshot.get(), &ModEntry32))
		{
			do {
			...
			} while (Module32Next(Snapshot.get(), &ModEntry32));
		}
	}
